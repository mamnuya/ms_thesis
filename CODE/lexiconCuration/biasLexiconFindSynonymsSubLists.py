...
bias_lexicon_path = "../../data/biasLexiconSubLists.json"
synonyms_bias_lexicon_path = "../../data/biasLexiconSynonymsSubLists.json"
...
def expand_lexicon(bias_lexicon):
    ...
    # Now process the categories within the "bias_lexicon" field
    categories = bias_lexicon.get("bias_lexicon", {})
    
    for category, fields in categories.items():  # Handle the structure correctly
        expanded_lexicon["bias_lexicon"][category] = {}

        # Process activities and descriptions separately
        for field_name in ["activities", "descriptions"]:
            words = fields.get(field_name, [])
            
            expanded_lexicon["bias_lexicon"][category][field_name] = set(words)  # Store as a set to avoid duplicates


            for word in words:
                tokenized_word, synonyms, synonym_tokens = get_filtered_synonyms(word)
                expanded_lexicon["bias_lexicon"][category][field_name].add(tokenized_word)  # Add tokenized word to the category
                expanded_lexicon["bias_lexicon"][category][field_name].update(synonyms)  # Add synonym to the same category
                all_synonym_tokens[word] = synonym_tokens  # Store synonym tokens for analysis

    # Convert sets to lists for JSON compatibility
    return {"bias_lexicon": {category: {field: list(words) for field, words in fields.items()} 
                             for category, fields in expanded_lexicon["bias_lexicon"].items()}}, all_synonym_tokens
