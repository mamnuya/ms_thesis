...
def compute_global_avg_distance(language_level_results):
    """Compute the global average of word-level Levenshtein distances across all languages."""
    complex_distances = []
    simple_distances = []
...
global_avg_distance = compute_global_avg_distance(language_level_results)

# Save the global average results
global_output_file = "../../../data/levenshtein_analysis/word_level/avg_bias_change_by_debiasing_method/global_average_word_level_levenshtein_distance.json"

with open(global_output_file, "w", encoding="utf-8") as f:
    json.dump(global_avg_distance, f, indent=4, ensure_ascii=False)

print(f"Global average word-level Levenshtein distance saved to {global_output_file}")